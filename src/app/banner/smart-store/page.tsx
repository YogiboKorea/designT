  ...
  useEffect(() => {
    fetch('/api/events')
      ...
      .then((d) => {
        if (d.success) {
          // 스마트스토어 배너만 필터
          const smartStoreBanners = (d.data as SavedBanner[]).filter(
            (item) => item.eventType === 'smart-store-banner'
          );
          setBanners(smartStoreBanners);
        }
      })
      .catch(console.error)
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
              <div style={{
                ...
                background: 'linear-gradient(135deg, #03c75a 0%, #1ec973 100%)',
              ...
              <span style={{
                ...
                background: '#ecfdf5', color: '#03c75a', borderRadius: '99px',
              }}>네이버</span>
            </div>
            <p style={{ margin: 0, fontSize: '14px', color: '#6b7280' }}>
              웹(1920×860)과 모바일(750×1350) 배너를 한 쌍으로 제작하고 관리합니다.
            </p>
          </div>
          <button
            onClick={() => router.push('/main-visual-smart-store')}
            style={{
              padding: '12px 24px', fontSize: '15px', fontWeight: 700,
              background: 'linear-gradient(135deg, #03c75a 0%, #1ec973 100%)',
              color: '#fff', border: 'none', borderRadius: '10px',
              cursor: 'pointer', boxShadow: '0 4px 12px rgba(3,199,90,0.3)',
            ...
            {banners.map((banner) => {
              ...
              return (
                ...
                >
                  {/* 썸네일 */}
                  <div style={{
                    width: '100%', aspectRatio: '1920/860',
                  ...
                  <div style={{ padding: '14px 16px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button
                        onClick={() => router.push(`/main-visual-smart-store?id=${banner._id}`)}
                        style={{
                          flex: 1, padding: '8px', fontSize: '13px', fontWeight: 600,
                          background: '#ecfdf5', color: '#03c75a',
